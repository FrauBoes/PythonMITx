...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) >= 1:  # If len(hand) > 0
        print('Current hand: ', end='')
        displayHand(hand)  # Print hand
        ...
        if word == '.':
            break  
            # If the input is a single period: End the game (break out of the
            #loop)
            
        else:  # Otherwise (the input is not a single period):
            if isValidWord(word, hand, wordList) is False:  
            # If word not valid: Reject invalid word (print a message followed 
            # by a blank line)
                print("Invalid word, please try again.")
                print()
            else:  # Otherwise (the word is valid):
                score += getWordScore(word, n)
                print('"', word, '"', 'earned', getWordScore(word, n), 'points. Total:', score, 'points', end=" ")  
                print()
